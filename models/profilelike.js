...
function getUsersLiked(userID) {
  let sql = `SELECT * FROM profilelike WHERE user_liked_profile = '${userID}'`;
  return promisifyQuery(sql);
}

function hasUserLiked(userID, otherUserID) {
  let sql = `SELECT COUNT(*) AS count FROM profilelike WHERE user_profile_liked = '${otherUserID}' AND user_liked_profile = '${userID}'`;
  return promisifyQuery(sql);
}
